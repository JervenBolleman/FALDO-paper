...
But where the INSDC record needs intelligent inspection before the data can be cleanly converted to a data model with rich semantics.
...
Like in the ASN.1 notation and datamodel used by GenBank each location in FALDO has an identifier for the sequence it is found on \cite{NCBI}.
Like in the GenBank data model this means that the position information is complete without further references to the context the position information was found in.
...
Only the UniProt RDF format allows other people to make assertions about UniProt data without breaking existing tools manner.
